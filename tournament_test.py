...
def test_pairings_odd():
    ...
    register_player("Applejack")
    register_player("Pinkie Pie")
    register_player("Odd Man")
    standings = player_standings()
    [id1, id2, id3, id4, id5] = [row[0] for row in standings]
    report_match(id1, id2)
    report_match(id3, id4)
    report_match(id5, None)
    pairings = swiss_pairings()
    if len(pairings) != 3:
        raise ValueError(
            "For five players, swiss_pairings should return three pairs.")
    ...
    if correct_pairs != actual_pairs:
        raise ValueError(
            "After one match, players with one win should be paired.")
    print "9. After one five-player match, players are correctly paired."
